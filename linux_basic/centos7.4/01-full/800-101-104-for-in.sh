...
#//      各执行一次(即 `for name` 默认是 `for arg in "$@"` 的简写形式), 整个复合命令的 return status
#//      就是 执行的最后命令的 exit  status. 如果 items 的 expansion 的结果是一个 empty list,
#//      那么 没有 command 会被执行，且 return status 为 0.
